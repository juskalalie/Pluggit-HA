...
class Pluggit:
    ...
    def __write_register(self, register: Registers, data: int):
        ...
        self.client.write_registers(
            address=item[0], values=builder.to_registers())
    ...
    def get_serial_number(self):
        low = self.__read_register(
            register=Registers.PRM_SYSTEM_SERIAL_NUM_LOW)
        high = self.__read_register(
            register=Registers.PRM_SYSTEM_SERIAL_NUM_HIGH)
    ...
    def get_filter_dirtiness(self):
        dirt = self.__read_register(
            register=Registers.PRM_FILTER_DIRTINESS_DEGREE)
    ...
    def get_bypass_actual_state(self):
        state = self.__read_register(
            register=Registers.PRM_RAM_IDX_BYPASS_ACTUAL_STATE)
    ...
    def set_unit_mode(self, mode: ActiveUnitMode):
        self.__write_register(
            register=Registers.PRM_RAM_IDX_UNIT_MODE, data=mode.value)
    ...
    def set_default_filter_time(self, time: int):
        self.__write_register(
            register=Registers.PRM_FILTER_DEFAULT_TIME, data=time)
    ...
    def set_bypass_tmin_summer(self, temp: float):
        self.__write_register(
            register=Registers.PRM_BYPASS_TMIN_SUMMER, data=temp)

    def set_bypass_tmax_summer(self, temp: float):
        self.__write_register(
            register=Registers.PRM_BYPASS_TMAX_SUMMER, data=temp)
